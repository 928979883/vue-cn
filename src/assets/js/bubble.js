...
class Vector2d {
  ...
  */
  ...
  /*
  * Subtract vector or return new vector 减去向量或返回新的向量
  * @param {constructor} v - vector or null
  * @param {number} x - vector x
  * @param {number} y - vector y
  */
  ...
  /* 
  * Multiple vector or return new vector 多重向量或返回新向量
  * @param {constructor} v - vector or null
  * @param {number} num
  */
  mult(v, num) {
  ...
  /*
  * Change vector from radian 从弧度改变向量
  * @param {number} radian
  */
  fromAngle(radian) {
...
class AnimationTimer {
  ...
  setTimeWarpFunction(timeWarp, option) {
    if (timeWarp === undefined || timeWarp === 'EaseInOut') {
      const arr = [
      'EaseIn',
      'EaseOut',
      'EaseInOut',
  ...
  */
  constructor(bool) {
    ...
    if (bool === true) {
      ...
      document.getElementsByTagName("section")[0].appendChild(this.canvas);

      /* document.getElementById("#ind02").appendChild(this.canvas); */
    }
